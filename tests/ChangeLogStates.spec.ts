...
describe("ChangeLogStates parse tests", () => {
  ...
  test('ChangeLogTitleState', () => {
    ...
    expect(ChangeLogTitleState.parse("###")).toBe(null)
  });

  test('ChangeLogDateState', () => {
    expect(ChangeLogDateState.parse("## Changelog 1.0")).toBe(null)
    expect(ChangeLogDateState.parse("### 1989-10-12")).toBe("1989-10-12")
    expect(ChangeLogDateState.parse("### 2153-10-12")).toBe("2153-10-12")
  });

  test('FeatureTitleState', () => {
    expect(FeatureTitleState.parse("## Changelog 1.0")).toBe(null)
    expect(FeatureTitleState.parse("#### Feature 1")).toBe("Feature 1")
  });

  test('FeatureDescriptionState', () => {
    expect(FeatureDescriptionState.parse("## Changelog 1.0")).toBe(null)
    expect(FeatureDescriptionState.parse("### 2099-03-21")).toBe(null)
    expect(FeatureDescriptionState.parse("#### Feature 1")).toBe(null)
